...
%%
function Output = TopOfClimbConstraintMission(inputs)
tolerance = 0.1;
...
WFcruise = WarmupTakeoffFunction(inputs).f_to.*ClimbFunction(inputs).f_cl;
  lapse = inputs.PropulsionInputs.lapse;

hc  = inputs.PerformanceInputs.hc;            % Altitude [ft]
[a,~,rho] = AtmosphereFunction(hc);
v = a*inputs.PerformanceInputs.M;
q = 0.5*rho*v^2;
...
%% Begin estimation of weight components (empty, fuel, and total weights)
  if initialFlag
    ...
    WSrange = inputs.PerformanceInputs.WS;
    AR = inputs.GeometryOutput.AR;

    inputs.PerformanceInputs.TW = WFcruise/lapse*(q/WFcruise*(Cdo./WSrange+WSrange./pi./AR./e0*(WFcruise/q)^2)+100/60/v);
